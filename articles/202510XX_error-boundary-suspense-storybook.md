...
この記事では、ErrorBoundary と Suspense を適切に配置することの重要性を、実装例を通じて紹介しました。

### ErrorBoundary と Suspense の配置がもたらす 3 つの価値
...
API をコールするコンポーネントの親コンポーネントに`<ErrorBoundary>`と`<Suspense>`を配置することで、エラーとローディングの範囲を限定できます。この原則により、テスト・開発体験・ユーザー体験のすべてが向上します。
