...
リクエストを中止する場合にキモとなってくるのは`AbortController`です（ちなみに英単語の abort は「中止する」や「中断する」といった意味があります）。この`AbortController`と`Axios`を紐づけることで画面遷移する際に通信を中断できます。具体的な実装は以下の通りで、通信を中止しない時と比べた場合のソースからの変更点は 3 箇所です。
